    ...
    let value = match target.obsidian_vault_id.as_deref().map(str::trim) {
        Some(id) if !id.is_empty() => {
            if id.len() != 16 || !id.chars().all(|character| character.is_ascii_hexdigit()) {
                return Err(AppError::new("VAULT_ID_INVALID", "Vault ID 应为 16 位十六进制字符。"));
            }
            id
        }
        _ => {
...
pub fn launch_obsidian_vault(app: &AppHandle, target: LaunchTarget) -> Result<String, AppError> {
    ...
    app.opener().open_url(uri.as_str(), None::<&str>).map_err(|_| {
        AppError::new(
...
#[cfg(test)]
mod tests {
    use super::{build_obsidian_uri, LaunchTarget};
    ...
    fn encodes_vault_name() {
        ...
        assert_eq!(uri.as_str(), "obsidian://open?vault=%E9%A1%B9%E7%9B%AE+%231+%26+%E8%B5%84%E6%96%99");
    }
