...
产品由桌面 Hub 与桌面专用 Bridge 插件组成。Hub 管理仓库入口与插件部署，Bridge 独立提供跨仓库浏览、链接和只读预览。用户于 2026-09-22 确认已有核心流程验收完成，当前重点为脱离 Hub 进程运行及链接可靠性。
...
- 完整的鼠标与键盘操作。
- Bridge 跨仓库目录浏览、元数据搜索、只读正文预览与 URI 跳转。
- Bridge 经授权安装、更新并启用；仅写自身插件目录与社区插件启用列表中的 Bridge 自身条目，不关闭受限模式、不改其他插件。Obsidian 运行时改写启用列表不即时生效，启用状态在下次启动体现。
- Bridge 只读使用 Hub 的版本化本机仓库登记表，Hub 退出不影响核心能力。

### MVP 不包含

- 自动扫描目录或全盘发现仓库。
- 跨仓库全文搜索、快速创建笔记和正文持久化索引。
- 修改其他插件、主题和工作区配置；修改用户笔记正文。
- Git、同步、容量或性能分析。
- 系统托盘、全局快捷键和自动更新。
- macOS 与 Linux 的正式支持。
...
- 默认只读访问仓库；除应用自己的配置文件与经授权安装/启用的 Bridge 插件文件外，不修改用户仓库内容。
- Bridge 通过独立本地访问接口读取登记仓库，索引只加速搜索，文件存在性以实时磁盘校验为准。HTTP 仅保留旧插件兼容与可选心跳，不是新插件的运行依赖。
- 现有 AppData `config.json`（schemaVersion 1）兼作仓库登记表，由 Hub 单写、Bridge 只读；不另建重复事实来源。
...
`apps/bridge/src/` 承载插件、本地读取接口及测试；`packages/protocol/` 承载共享 DTO，`packages/cross-vault-parser/` 承载唯一链接语法实现。新本地读取模块不得使用通用 shell 或写入其他仓库。
...
- 新链接使用可读名称加稳定 Hub ID 标识仓库，旧名称链接继续兼容；同名多解明确报错。所有路径检查真实路径与登记根目录边界，拒绝符号链接逃逸。
- 无 Vault ID 时可使用官方 URI 的绝对文件 path 参数；仍须验证文件和仓库边界，不以展示名猜测目标。
